...
def handle_agent_action_core(
    ...
    # Verify tool execution authenticity before accepting result
    try:
            ...
            with verifier.lock:
                if token_id in verifier.execution_registry:
                    ...
                    if verifier.verify_execution(token, clean_result):
                        # Authentic execution - use clean result
                        tool_result.result = clean_result
                    else:
                        # Invalid token - flag as fabricated
                        print(f"⚠️ INVALID TOKEN DETECTED: {tool_result.result[:100]}...")
                        tool_result.result = "ERROR: Tool execution token invalid. Please try again."
                else:
                    # Token not found - flag as fabricated
                    print(f"⚠️ UNKNOWN TOKEN DETECTED: {tool_result.result[:100]}...")
                    tool_result.result = "ERROR: Tool execution token not found. Please try again."
        else:
            # No token present - likely fabricated
        ...
        # Graceful degradation if verification module not available
        pass
