...
class ToolExecutionMonitor:
    ...
    def start_monitoring(self, directory: str = ".") -> None:
        ...
        # Capture baseline filesystem state - but limit to avoid performance issues
        self.baseline_filesystem_state = set()
        try:
            # For performance, only monitor current directory and common output directories
            monitor_paths = [directory]
            if directory == ".":
                # Add common output directories but avoid walking entire source tree
                monitor_paths = [".", "tmp", "output", "temp"]
            
            for monitor_dir in monitor_paths:
                if not os.path.exists(monitor_dir):
                    continue
                
                # Only walk one level deep for performance
                try:
                    if os.path.isfile(monitor_dir):
                        # Single file
                        stat = os.stat(monitor_dir)
                        self.baseline_filesystem_state.add(f"{monitor_dir}:{stat.st_mtime}")
                    else:
                        # Directory - limit depth
                        for item in os.listdir(monitor_dir):
                            filepath = os.path.join(monitor_dir, item)
            ...
            logger.debug(f"Filesystem monitoring limited due to permissions: {e}")
    ...
    def stop_monitoring_and_verify(self, tool_name: str, tool_result: Any, directory: str = ".") -> ToolExecutionCertificate:
        ...
        # Check for filesystem changes - using same efficient approach as start_monitoring
        try:
            ...
            for monitor_dir in monitor_paths:
                ...
                try:
                    if os.path.isfile(monitor_dir):
                        # Single file
                        stat = os.stat(monitor_dir)
                        current_state = f"{monitor_dir}:{stat.st_mtime}"
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        total_executions = sum(self.authenticity_counts.values())
        if total_executions == 0:
            return {"total_executions": 0, "authenticity_rates": {}}
        
        authenticity_rates = {
            level: count / total_executions 
            for level, count in self.authenticity_counts.items()
        }
        
        return {
            "total_executions": total_executions,
            "authenticity_counts": self.authenticity_counts.copy(),
            "authenticity_rates": authenticity_rates,
            "fabrication_rate": authenticity_rates.get("likely_fake", 0) + authenticity_rates.get("verified_fake", 0),
            "authentic_rate": authenticity_rates.get("likely_real", 0) + authenticity_rates.get("verified_real", 0)
        }
