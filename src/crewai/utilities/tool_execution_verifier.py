...
class ToolExecutionMonitor:
    ...
    def __init__(self):
        ...
        self.baseline_filesystem_state: set[str] | None = None
        import tempfile
        self.monitoring_temp_dir = Path(tempfile.gettempdir()) / "crewai_tool_monitoring"
    ...
    def capture_subprocess_spawn(self, execution_id: str, pid: int) -> None:
    ...
    def capture_filesystem_change(self, execution_id: str, file_path: str) -> None:
    ...
    def _generate_execution_id(self, tool_name: str, tool_args: dict[str, Any]) -> str:
        ...
        return hashlib.sha256(id_data.encode()).hexdigest()
    
    def _capture_baseline_filesystem_state(self) -> None:
    ...
    def _detect_filesystem_changes(self, execution_id: str, evidence: ExecutionEvidence) -> None:
...
class ToolExecutionVerifier:
    ...
    def __init__(self):
        ...
        # Hook into CrewAI events (delayed import to avoid circular imports)
        try:
            from crewai.utilities.events.crewai_event_bus import CrewAIEventsBus
            from crewai.utilities.events.tool_usage_events import ToolUsageFinishedEvent
            
            event_bus = CrewAIEventsBus()
            event_bus.on(ToolUsageFinishedEvent)(self._on_tool_usage_finished)
        except ImportError:
            # If events system isn't available, verification still works
            pass
...
_global_verifier: ToolExecutionVerifier | None = None
...
def enable_strict_verification() -> None:
...
if __name__ == "__main__":
    ...
    import tempfile
    temp_file = Path(tempfile.gettempdir()) / "real_file.txt"
    verifier.monitor.capture_filesystem_change("test_exec", str(temp_file))
    real_result = f"File written to {temp_file}"
